...
deploy = '/root/caffe/models/bvlc_googlenet/deploy.prototxt'  # deploy文件
...
test_image = root + '/material/BOT/test/dog.jpg'  # 测试图片所在的文件路径
output_path = root + '/data/BOT/visual/'
...
def show_data(data, output_name):
    ...
    plt.axis('off')
    plt.savefig(output_path + output_name + '.png', format='png')
    # plt.show()

def show_feature(layer,outputname):
    ...
    show_data(net.blobs[layer].data[0], outputname + '_blobs')
    if 'conv' in layer:    
	show_data(net.params[layer][0].data.transpose(0, 2, 3, 1), outputname + '_params')

# ！显示减去均值前后的图片
def show_contrast(output_name):
    plt.figure()
...
# 直线
def draw_line(prob,output_name):
    plt.figure(3)
    plt.plot(prob)
    plt.title('Test', size=16)
    plt.savefig(output_path + output_name + '.png', format='png')
...
def predict(form = '',output_name = ''):
    ...
    # prob = [1,2,5,6,8,4,5,65,65,6,11,12]
    if form == '':
	return
    elif form == 'pie': 
	draw_pie(prob,output_name)
    elif form == 'bar':
	draw_bar(prob,output_name)
    elif form == 'line':
	draw_line(prob,output_name)
    else:
	print '\nERROR: no such form'
...
if __name__ == '__main__':
  ...
  if param == 'blob':
	...
	show_feature(sys.argv[i],sys.argv[i+1])     	
  else:
    print '\nERROR: no such argvs'
